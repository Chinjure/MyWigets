...
$src = Join-Path $root 'src\calendar_main.cpp'
$outDir = Join-Path $root 'bin'
$null = New-Item -ItemType Directory -Force -Path $outDir
$exe = Join-Path $outDir "DesktopCalendar-$Architecture.exe"
...
$clLine = "cl /nologo /std:c++17 /EHsc $optim /W4 /permissive- /utf-8 $defs /Fe:`"$exe`" `"$src`" /link /SUBSYSTEM:WINDOWS gdiplus.lib user32.lib gdi32.lib advapi32.lib"

$bat = Join-Path $env:TEMP 'desktopcalendar_build.bat'
