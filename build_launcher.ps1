...
$src = Join-Path $root 'src\launcher_main.cpp'
$rc = Join-Path $root 'src\app.rc'
$srcDir = Join-Path $root 'src'
$outDir = Join-Path $root 'bin'
$null = New-Item -ItemType Directory -Force -Path $outDir
$exe = Join-Path $outDir "DesktopLauncher-$Architecture.exe"
$res = Join-Path $outDir "desktoplauncher-$Architecture.res"
...
$clLine = "cl /nologo /std:c++17 /EHsc $optim /W4 /permissive- /utf-8 $defs /Fe:`"$exe`" `"$src`" `"$res`" /link /SUBSYSTEM:WINDOWS gdiplus.lib user32.lib gdi32.lib advapi32.lib shell32.lib comctl32.lib ole32.lib"

$bat = Join-Path $env:TEMP 'desktoplauncher_build.bat'
